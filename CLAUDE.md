...
- **LINE 生產登記**（2026-07-10新增，`handleLineProductionRecord`）：員工傳「生產 商品名稱 數量」（如「生產 滷味拼盤18份」），AI 從有設定「配方」的商品清單裡比對名稱，依配方（做1單位要用多少原物料）× 數量算出實際消耗量，呼叫既有 `saveProductionRecord` 扣原物料庫存、加商品庫存，回覆消耗明細；若消耗後原物料庫存會不足，回覆訊息會標示 ⚠️ 警告但仍照常記錄（比照網頁版「生產紀錄」頁行為，不阻擋）
