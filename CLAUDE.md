...
- **B1 補給官**：✅ 已上線。LINE 進貨記帳當下即時比對歷史均價，漲幅超過 10% 推播示警
  ...
  - LINE 進貨記帳新增金額防呆（2026-07-10新增）：已比對到原物料且有填數量，但金額為 0 或未填時，本次不記帳並回覆提醒補打金額（避免「有數量沒金額」的訊息如「品名 2桶」被靜靜記成 $0 成本），比照既有「沒打數量」的擋法邏輯
...
- **LINE 生產登記**（2026-07-10新增，`handleLineProductionRecord`）：員工傳「生產 商品名稱 數量」（如「生產 滷味拼盤18份」），AI 從有設定「配方」的商品清單裡比對名稱，依配方（做1單位要用多少原物料）× 數量算出實際消耗量，呼叫既有 `saveProductionRecord` 扣原物料庫存、加商品庫存，回覆消耗明細；若消耗後原物料庫存會不足，回覆訊息會標示 ⚠️ 警告但仍照常記錄（比照網頁版「生產紀錄」頁行為，不阻擋）。回覆訊息裡的數字用 `round1_()` 四捨五入到小數點一位，避免浮點數運算誤差（如 300.59999999999997）直接顯示給員工看
